defmodule SistemaExperto.Learning do
  ...
  muevo_peso =Enum.reduce(1..150, matriz_pesos, fn _, matriz_pesos ->
  ...
  IO.inspect(matriz, label: "Matriz de Activación")
    ...
    # Crea un nuevo vector donde el valor máximo es 1 y los otros son 0
  ...
  def matriz_activacion(caso, centros_anchos,matriz_peso)do
     ...
     matriz_activacion =for (centroAnchos <-centros_anchos)do
...
# calcula el ancho
  ...
  #calcula la media de los vecinos
  def vecinos_media(centro, vecinos) do


    # Filtra y calcula las distancias
    ...
    # Validar que `distancias` sean listas planas
  ...
  def calcular_mediana(cluster, vecinos) do
    Enum.map(vecinos,fn vecino -> SistemaExperto.Kmean.distancia_euclidiana([cluster,vecino])end)
  end
  #calculo de la funcion gauseana
 ...
 #con esto genero una matriz aleatoria
...
#con esto tambien multiplico matrices
  def maltiplicacion(lista,activacion)do
    ...
    formato =for (lista_formato <-ordenada)do
  ...
  #con esto calculo el error
  def croos_entropy(resultado_real,resultado_esperado )
  do
    resultado =Enum.zip(resultado_real,resultado_esperado)
  ...
  def derivada_funcion_gauseana(centros_anchos, entrada)do
    ...
    Enum.map(centros_anchos,fn [centro, ancho] -> -1*((distancia_euclidia(centro,entrada)*(ancho**2))*:math.exp((-(distancia_euclidia(centro,entrada)**2))/2*(ancho**2)))end)

  end
  #con esto multiplico matrices
  ...
  #con esto multiplico matrices por escalares
...
matriz =Enum.zip(matriz1,matriz2)
|>Enum.map(&Tuple.to_list/1)
Enum.map(matriz, fn [fila1,fila2] ->Enum.zip(fila1,fila2)|>Enum.map(&Tuple.to_list/1)
|>Enum.map(fn [elemento1,elemento2] -> elemento1-elemento2

end)
end)
end
 # def reamdomizacion_de_entremianto(data)do
 # random_elements = for _ <- 1..150 do  Enum.random(data) end
 # end
